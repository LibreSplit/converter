    ...
    pub fn convert_icon(source: &str) -> Result<String, String> {
        ...
        if wrapped || encoded.starts_with(b"AAEAAAD/////") {
            ...
            let decoded = STANDARD
                .decode(&encoded)
                .map_err(|error| format!("Invalid base64 in embedded icon: {error}"))?;
            ...
            let image = decoded
                ...
                .find_map(|(offset, record)| {
                    ...
                    if decoded.get(end..) != Some(&[11][..])
                        || !decoded[..offset]
                            .windows(bitmap_type.len())
                            .any(|text| text == bitmap_type)
                    {
                        return None;
                    }
                ...
                .or_else(|| {
                    for event in EventReader::new(image) {
                        if let XmlEvent::StartElement { name, .. } = event.ok()? {
                            return (name.local_name == "svg"
                                && name.namespace.as_deref().is_none_or(|namespace| {
